...
type FeedPostType = PostType & {user: User} & {likes:[{userId:string}]} & {_counts:{comments:number}}
...
const Post = ({post}:{post: FeedPostType}) => {
  return (
    <div className='fle flex-col gap-4'>
        ...
        {/* INTERACTION */}
        <PostInteraction postId={post.id} likes={post.likes.map((like) => like.userId)} commentNumber={post._counts.comments} />
        <Comments />
    </div>
  )
}
